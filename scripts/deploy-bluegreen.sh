...
YES=false
usage(){ cat <<EOF
Usage: $0 [--clean] [--yes] [--namespace <ns>] [--base-dir <path>] [--hostpath-base <path>] [--temp-dir <path>]
EOF
}
...
while [[ $# -gt 0 ]]; do case "$1" in --clean) CLEAN=true; shift ;; --yes) YES=true; shift ;; --namespace|-n) NAMESPACE="$2"; shift 2 ;; --base-dir) BASE_DIR="$2"; shift 2 ;; --hostpath-base) HOSTPATH_BASE="$2"; shift 2 ;; --temp-dir) TEMP_BASE_DIR="$2"; shift 2 ;; -h|--help) usage; exit 0 ;; *) echo "Unknown option: $1" >&2; exit 1 ;; esac; done
