...
kubectl patch service tileserver -n "$NAMESPACE" --type=json -p="[{"op":"replace","path":"/spec/selector/color","value":"${TARGET_COLOR}"},{"op":"replace","path":"/spec/selector/active","value":"true"}]"
...
kubectl patch deployment "tileserver-${TARGET_COLOR}" -n "$NAMESPACE" --type=json -p="[{"op":"replace","path":"/spec/template/metadata/labels/active","value":"true"}]"
kubectl patch deployment "tileserver-${OLD_COLOR}" -n "$NAMESPACE" --type=json -p="[{"op":"replace","path":"/spec/template/metadata/labels/active","value":"false"}]" || true
