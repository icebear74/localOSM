...
DATA_DIR = os.environ.get("OSM_DATA_DIR", "/mnt/OSM")
TEMP_DIR = os.environ.get("OSM_TEMP_DIR", "/mnt/OSM/TempDir")
...
VALHALLA_TILE_DIR = os.path.join(DATA_DIR, "valhalla", "tiles")
...
# import-orchestrator.sh writes its own live progress here (same shared state path as
...
# MapLibre GL base style – kept in sync with k8s/tileserver-gl-deployment.yaml init container.
_STYLE_JSON = (
    ...
    '{"id":"water-name","type":"symbol","source":"openmaptiles","source-layer":"water_name","layout":{"text-field":["coalesce",["get","name:de"],["get","name:en"],["get","name"]],"text-font":["NotoSansRegular"],"text-size":{"stops":[[4,10],[10,14]]},"symbol-placement":"point"},"paint":{"text-color":"#5b8db8","text-halo-color":"#fff","text-halo-width":1}},'
    ...
    '{"id":"place-country","type":"symbol","source":"openmaptiles","source-layer":"place","filter":["==","class","country"],"layout":{"text-field":["coalesce",["get","name:de"],["get","name:en"],["get","name"]],"text-font":["NotoSansRegular"],"text-size":{"stops":[[2,10],[6,16],[8,18]]}},"paint":{"text-color":"#333","text-halo-color":"rgba(255,255,255,0.8)","text-halo-width":2}},'
    '{"id":"place-state","type":"symbol","source":"openmaptiles","source-layer":"place","filter":["==","class","state"],"minzoom":5,"maxzoom":12,"layout":{"text-field":["coalesce",["get","name:de"],["get","name:en"],["get","name"]],"text-font":["NotoSansRegular"],"text-size":{"stops":[[5,10],[10,14]]}},"paint":{"text-color":"#555","text-halo-color":"rgba(255,255,255,0.8)","text-halo-width":1.5}},'
    '{"id":"place-city","type":"symbol","source":"openmaptiles","source-layer":"place","filter":["in","class","city"],"layout":{"text-field":["coalesce",["get","name:de"],["get","name:en"],["get","name"]],"text-font":["NotoSansRegular"],"text-size":{"stops":[[6,11],[10,15],[14,18]]}},"paint":{"text-color":"#222","text-halo-color":"rgba(255,255,255,0.9)","text-halo-width":2}},'
    '{"id":"place-town","type":"symbol","source":"openmaptiles","source-layer":"place","filter":["==","class","town"],"minzoom":8,"layout":{"text-field":["coalesce",["get","name:de"],["get","name:en"],["get","name"]],"text-font":["NotoSansRegular"],"text-size":{"stops":[[8,10],[12,14],[16,17]]}},"paint":{"text-color":"#333","text-halo-color":"rgba(255,255,255,0.9)","text-halo-width":1.5}},'
    '{"id":"place-village","type":"symbol","source":"openmaptiles","source-layer":"place","filter":["==","class","village"],"minzoom":10,"layout":{"text-field":["coalesce",["get","name:de"],["get","name:en"],["get","name"]],"text-font":["NotoSansRegular"],"text-size":{"stops":[[10,10],[14,13]]}},"paint":{"text-color":"#444","text-halo-color":"rgba(255,255,255,0.9)","text-halo-width":1.5}},'
...
<body>
  ...
  function updateLinks() {
    ...
    // Point Maputnik at TileServer-GL's own resolved style endpoint, not the raw
    // stored style.json (served via /api/style): the stored file's source uses
    // the TileServer-GL-internal "mbtiles://{v3}" placeholder, which TileServer-GL
    // itself rewrites into real tile/TileJSON URLs when serving /styles/<id>/style.json.
    // Loading the raw file directly left Maputnik with an unresolvable source, so
...
def _format_mtime(path):
    try:
        return datetime.fromtimestamp(os.path.getmtime(path)).astimezone().strftime("%Y-%m-%d %H:%M %Z")
    except OSError:
        return ""
...
def _copy_preserving_ownership(src, dst):
    """shutil.copytree() the tree from src to dst, then restore ownership.

    shutil.copy2()/copytree() preserve mode and timestamps but never call
    chown(), so every copied file/dir ends up owned by whatever user this
    process runs as (root in the status pod). For Nominatim's Postgres data
    directory that broke ownership set up during import (postgres:postgres),
    causing "Data directory ... must not be owned by root" and a crash-loop
    right after promoting. Walk src (still present at this point) and mirror
    its uid/gid onto the corresponding copied path in dst.
    """
    ...
    for root, dirs, files in os.walk(src):
        ...
        try:
            st = os.stat(root)
            os.chown(target_root, st.st_uid, st.st_gid)
        except OSError:
            pass


def _move_directory_replacing(src, dst):
    """Move directory src to dst, tolerating cross-device mounts.

    os.replace() (rename(2)) requires src and dst to live on the same
    filesystem/device. staging_dir (under OSM_TEMP_DIR) and active_dir (under
    OSM_DATA_DIR) are mounted as separate hostPath volumes in Kubernetes, so
    even though they may point at paths on the same physical disk, the
    container sees them as different devices and rename() fails with
    "[Errno 18] Invalid cross-device link". Fall back to a copy+delete move
    in that case, preserving ownership (see _copy_preserving_ownership).
    """
    try:
        os.replace(src, dst)
    except OSError as exc:
        if exc.errno != errno.EXDEV:
            raise
        _copy_preserving_ownership(src, dst)
        try:
            shutil.rmtree(src)
        except OSError:
...
def _restore_missing_entries(backup_dir, active_dir):
    """Copy back entries from backup_dir that are missing in active_dir.

    Used after promoting tileserver staging data (which only contains the
    freshly built map.mbtiles) so that files/directories that are not part
...
def promote_service(service):
    ...
    if had_active:
        _move_directory_replacing(active_dir, backup_dir)

    try:
        _move_directory_replacing(staging_dir, active_dir)
    except OSError as exc:
        if os.path.isdir(backup_dir):
            if os.path.isdir(active_dir):
                shutil.rmtree(active_dir)
            _move_directory_replacing(backup_dir, active_dir)
        raise RuntimeError(f"Could not promote staged {service} data: {exc}") from exc

    if service == "tileserver" and had_active and os.path.isdir(backup_dir):
        try:
            _restore_missing_entries(backup_dir, active_dir)
        except OSError as exc:
            raise RuntimeError(
    ...
    if os.path.isdir(IMPORT_DIR):
        for entry in sorted(os.listdir(IMPORT_DIR)):
            ...
            try:
                ...
                mtime = datetime.fromtimestamp(os.path.getmtime(full_path)).astimezone().strftime("%Y-%m-%d %H:%M %Z")
            except OSError:
                size_mb, mtime = 0, "?"
...
def merge_library_files(country, records=None):
    ...
    if len(input_paths) == 1:
        ...
        # Country extracts are downloaded/cached independently and at different
        # times, so shared border nodes/ways can end up with different edit
        # versions in each file. "osmium merge" (the correct tool for combining
        # several extracts, as opposed to "osmium cat") keeps every distinct
        # version it finds instead of silently dropping one, which means the
        # merged file can still contain the same node/way/relation ID more than
        # once. osm2pgsql (used by the Nominatim import) is not history-aware
        # and aborts with "Input data is not ordered: ... appears more than
        # once" in that case. Collapsing the merged file with "osmium
        # time-filter" (no explicit timestamp = current point in time) keeps
        # only the latest version of each object, guaranteeing a clean,
        # duplicate-free extract regardless of how stale the individual
        # cached/downloaded files were relative to each other.
        run_command(["osmium", "merge", "--overwrite", "-o", tmp_path, "-f", "pbf", *input_paths], timeout=7200)
        dedup_output_path = f"{MERGED_PBF}.dedup"
        run_command(["osmium", "time-filter", "--overwrite", "-o", dedup_output_path, "-f", "pbf", "-F", "pbf", tmp_path], timeout=7200)
        os.replace(dedup_output_path, tmp_path)
